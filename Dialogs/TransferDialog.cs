...
namespace Banko.Dialogs
{
    ...
    public class TransferDialog : DialogSet
    {
            ...
            Add(nameof(TransferDialog), new WaterfallStep[]
            {
                ...
                async (dc, args, next) =>
                {
                    ...
                    await dc.Prompt(Keys.Confirm,
                        $"Ok. I'll make this transfer " +
                        $"£{dc.ActiveDialog.State[Keys.Money]} " +
                        $"from {dc.ActiveDialog.State[Keys.AccountLabel]} " +
                        $"to {dc.ActiveDialog.State[Keys.Payee]} " +
                        $"on {dc.ActiveDialog.State[Keys.Date]}?", new PromptOptions
                        {
                            RetryPromptString = "I'm sorry, should I make the transfer for you? " +
        ...
        private Dictionary<string, object> ValidateLuisArgs(BankoLuisModel._Entities entities)
        {
            ...
            // Check Date. Hardcoded for now
            result[Keys.Date] = DateTime.UtcNow;
